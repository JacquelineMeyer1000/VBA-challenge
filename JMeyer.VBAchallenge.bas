Attribute VB_Name = "Module1"
Sub StockData()
    
    ' LOOP THROUGH ALL SHEETS
    Dim WS As Worksheet
    
    For Each WS In ActiveWorkbook.Worksheets
    WS.Activate
        
        ' Determine the Last Row
        LastRow = WS.Cells(Rows.Count, 1).End(xlUp).Row
        
        'Create Variables to hold value
        Dim Open_Price As Double
        Dim Close_Price As Double
        Dim Yearly_Change As Double
        Dim Ticker_Name As String
        Dim Percent_Change As Double
        Dim Volume As Double
            Volume = 0
        Dim Row As Double
            Row = 2
        Dim Column As Integer
            Column = 1
        Dim i As Long
        
        ' Add heading + bold face for summary (incl. auto fit)
        WS.Cells(1, "I").Value = "Ticker"
        WS.Cells(1, "J").Value = "Yearly Change"
        WS.Cells(1, "K").Value = "Percent Change"
        WS.Cells(1, "L").Value = "Total Stock Volume"
        
        WS.Range("I1:L1").Font.Bold = True
        WS.Range("I1:L1").Columns.AutoFit
        
        ' Set Initial Open Price and loop through ticker column
        Open_Price = Cells(2, Column + 2).Value
         
                For i = 2 To LastRow
                    
                    'if cells are a different ticker ---->
                    If Cells(i + 1, Column).Value <> Cells(i, Column).Value Then
                        
                        ' Ticker name
                        Ticker_Name = Cells(i, Column).Value
                        Cells(Row, Column + 8).Value = Ticker_Name
                
                        ' Close Price
                        Close_Price = Cells(i, Column + 5).Value
                
                        ' Yearly Change
                        Yearly_Change = Close_Price - Open_Price
                        Cells(Row, Column + 9).Value = Yearly_Change
                        
                        ' Percent Change
                                If (Open_Price = 0 And Close_Price = 0) Then
                                    Percent_Change = 0
                                    
                                        ElseIf (Open_Price = 0 And Close_Price <> 0) Then
                                            Percent_Change = 1
                                                
                                        Else
                                            Percent_Change = Yearly_Change / Open_Price
                                            Cells(Row, Column + 10).Value = Percent_Change
                                            Cells(Row, Column + 10).NumberFormat = "0.00%"
                                
                                End If
                            
                        ' Total Volume
                        Volume = Volume + Cells(i, Column + 6).Value
                        Cells(Row, Column + 11).Value = Volume
                        Volume = 0
                        Row = Row + 1
                        Open_Price = Cells(i + 1, Column + 2)
                        
            
                        '----> Else, cells are the same ticker
                        Else
                            Volume = Volume + Cells(i, Column + 6).Value
                    
                    End If
                Next i
        
        ' Determine the Last Row of Yearly Change (YC) per WS
        YCLastRow = WS.Cells(Rows.Count, Column + 8).End(xlUp).Row
        
            ' Set the Cell Colors
            For j = 2 To YCLastRow
                
                If (Cells(j, Column + 9).Value > 0 Or Cells(j, Column + 9).Value = 0) Then
                    Cells(j, Column + 9).Interior.ColorIndex = 4
                
                    ElseIf Cells(j, Column + 9).Value < 0 Then
                        Cells(j, Column + 9).Interior.ColorIndex = 3
                
                End If
            Next j
            
            ' Loop through each rows to find the greatest value and its associate ticker
            For Z = 2 To YCLastRow
                
                    If Cells(Z, Column + 10).Value = Application.WorksheetFunction.Max(WS.Range("K2:K" & YCLastRow)) Then
                        Cells(2, Column + 15).Value = Cells(Z, Column + 8).Value
                        Cells(2, Column + 16).Value = Cells(Z, Column + 10).Value
                        Cells(2, Column + 16).NumberFormat = "0.00%"
                        
                            ElseIf Cells(Z, Column + 10).Value = Application.WorksheetFunction.Min(WS.Range("K2:K" & YCLastRow)) Then
                                Cells(3, Column + 15).Value = Cells(Z, Column + 8).Value
                                Cells(3, Column + 16).Value = Cells(Z, Column + 10).Value
                                Cells(3, Column + 16).NumberFormat = "0.00%"
                                
                            ElseIf Cells(Z, Column + 11).Value = Application.WorksheetFunction.Max(WS.Range("L2:L" & YCLastRow)) Then
                                Cells(4, Column + 15).Value = Cells(Z, Column + 8).Value
                                Cells(4, Column + 16).Value = Cells(Z, Column + 11).Value
                        
                    End If
            Next Z
            
        ' Set Greatest % Increase, % Decrease, and Total Volume along with font, color fill, & border
        WS.Cells(2, Column + 14).Value = "Greatest % Increase"
        WS.Cells(3, Column + 14).Value = "Greatest % Decrease"
        WS.Cells(4, Column + 14).Value = "Greatest Total Volume"
        WS.Cells(1, Column + 15).Value = "Ticker"
        WS.Cells(1, Column + 16).Value = "Value"
        
        WS.Range("O2:O4, P1, Q1").Font.Bold = True
        WS.Range("O1:Q1, O4:Q4").Borders(xlEdgeBottom).LineStyle = XlLineStyle.xlContinuous
        WS.Range("O1:O4").Borders(xlEdgeLeft).LineStyle = XlLineStyle.xlContinuous
        WS.Range("Q1:Q4").Borders(xlEdgeRight).LineStyle = XlLineStyle.xlContinuous
        
        WS.Range("O1:O4, Q1:Q4, P1:P4").Select
        Selection.Interior.ColorIndex = 27
        
        WS.Range("O1:O4").Columns.AutoFit
            
    Next WS
    
End Sub
    

